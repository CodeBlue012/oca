    ...
    extends Mage_Core_Model_Abstract
{
    ...
    public function getThumbnail($field_id, $filename, $width, $height = null)
    {
        ...
        if (file_exists($imageUrl)) {
            ...
            if (!file_exists($imageResized) || Mage::getStoreConfig('webforms/images/cache') == 0) {
                ...
                $this->setMemoryForImage($imageUrl);
        ...
        $url = Mage::app()->getStore($this->getStoreId())->getBaseUrl(Mage_Core_Model_Store::URL_TYPE_MEDIA) . "webforms/" . $this->getId() . "/" . $field_id;
        if ($this->getData('key_' . $field_id)) {
    ...
    {
        $filename = Varien_File_Uploader::getCorrectFileName($filename);
        $path = Mage::app()->getStore($this->getStoreId())->getBaseUrl(Mage_Core_Model_Store::URL_TYPE_MEDIA) . 'webforms/' . $this->getId() . '/' . $field_id . '/';
        if ($this->getData('key_' . $field_id)) {
            $path .= $this->getData('key_' . $field_id) . '/';
        }
        ...
        foreach ($this->getWebform()->getFieldsToFieldsets(true) as $fieldset_id => $fieldset) {
            foreach ($fieldset["fields"] as $field) {
                ...
                if ($field->getType() == 'image' && $resize && ($width > 0 || $height > 0)) {
                    $filename = Varien_File_Uploader::getCorrectFileName($this->getData('field_' . $field->getId()));
                    $imageUrl = $this->getFileFullPath($field->getId(), $filename);
                    $file_info = @getimagesize($imageUrl);
                    if ($file_info) {
                        // skip bmp files
                        if (!strstr($file_info["mime"], "bmp")) {
