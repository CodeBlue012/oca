    ...
    public function processEvents($observer)
    {
        ...
        if (!$disabled) {
            foreach ($this->_queueManager->getOldestEvents($registration->getConnectorKey()) as $event) {
                ...
                if (!is_null($data) && array_key_exists('context', $data['data'])) {
                ...
                $key = 'error';
                if (!is_null($data) && $this->_platform->dispatch($data)) {
                    $key = 'success';
        ...
        $observer->getEndpoint()->addExtension(array(
            ...
            'fields' => array(
                ...
                array(
                    ...
                    'type' => 'boolean',
                    'required' => true,
                    'allowedScopes' => array( $observer->getRegistration()->getScopeHash() ),
                    'depends' => array(
